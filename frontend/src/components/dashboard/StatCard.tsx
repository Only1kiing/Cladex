    ...
    >
      ...
      {trend && (
        <div className="flex items-center gap-1 mt-2">
          {trend.direction === 'up' ? (
            <svg width="14" height="14" viewBox="0 0 14 14" fill="none" className="text-nova-400">
              <path d="M7 2.5L12 8.5H2L7 2.5Z" fill="currentColor" />
            </svg>
          ) : (
            <svg width="14" height="14" viewBox="0 0 14 14" fill="none" className="text-apex-400">
              <path d="M7 11.5L2 5.5H12L7 11.5Z" fill="currentColor" />
            </svg>
          )}
          <span
            className={`text-xs font-medium ${
              trend.direction === 'up' ? 'text-nova-400' : 'text-apex-400'
            }`}
          >
            {trend.value}
          </span>
